# Install CycleInfraLnd package to wotk with TFL provided dataset of infrastructure.
...
# Import boroughs boundaries map. 
# (ONS London data - file = lon_boundaries_Dec_2020_BFC.Rds)
...
# Check for missing values.                    
nan_borough <- cycle_cross[is.na(cycle_cross$BOROUGH),]
sum(is.na(cycle_cross))   # None found.
...
# No map image, as there are too many signs and it complicates the script. 
# You can filter the dataset by some criteria an see the distribution on the map.
...
### Code, if needed to see signs by filtering for borough for example.
# Richmond = cycle_sign %>% 
#  filter(BOROUGH == "Richmond upon Thames")
# mapview(Richmond, cex=1) +
#  mapview(lon_2020, alpha.regions = 0.3, zcol = "BOROUGH", legend = FALSE, lwd = 0.5)
